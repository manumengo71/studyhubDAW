    ...
    <div class="flex justify-between items-center">
        <form class="flex-1 m-4">
            ...
            <div class="relative">
                ...
                <input type="search" id="default-search" class="block w-full p-4 pl-10 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Buscar cursos..." required>
            ...
            @if ($courses->where('courses_categories_id', $tema->id)->isNotEmpty())
                <div class="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4">
                    ...
                    <div id="animation-carousel-{{ $tema->id }}" class="relative w-auto mx-4 mt-5 mb-5"
                        ...
                        <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                            @foreach ($courses->where('courses_categories_id', $tema->id) as $course)
                                   ...
                                   @if ($course->getMedia('courses_images')->count() > 0)
                                    <div class="relative">
                                        <img src="{{ $course->getMedia('courses_images')->last()->getUrl() }}"
                                            ...
                                            alt="...">
                                        <div class="absolute block w-50 -translate-x-1/2 -translate-y-1/2 bottom-5 left-1/2 bg-black opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                            <h2 class="text-2xl font-bold text-white">{{ $course->name }}</h2>
                                        </div>
                                    </div>
                                @else
                                    <img src="https://i.postimg.cc/HkL86Lc1/sinfoto.png"
                                        class="absolute block w-60 -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
                                        alt="...">
                                        <div class="absolute block w-50 -translate-x-1/2 -translate-y-1/2 bottom-5 left-1/2 bg-black opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                            <h2 class="text-2xl font-bold text-white">{{ $course->name }}</h2>
                                        </div>
                                @endif
                                    <div
                                        class="absolute block w-50 -translate-x-1/2 -translate-y-1/2 bottom-5 left-1/2 bg-black">
                                        <h2 class="text-2xl font-bold text-white">{{ $course->name }}</h2>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <button type="button"
                        ...
                        data-carousel-prev>
                        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-blue-500/30 dark:bg-blue-800/30 group-hover:bg-blue-500/50 dark:group-hover:bg-blue-800/60 group-focus:ring-4 group-focus:ring-blue-500 dark:group-focus:ring-blue-800/70 group-focus:outline-none">
