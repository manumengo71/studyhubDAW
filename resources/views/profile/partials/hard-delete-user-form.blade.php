<section class="space-y-6">
    ...
    <x-danger-button
        x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirma-eliminacion-cuenta')"
    >{{ __('Eliminar cuenta') }}</x-danger-button>

    <x-modal name="confirma-eliminacion-cuenta" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('userprofile.destroy') }}" class="p-6">
            @csrf
            @method('delete')
