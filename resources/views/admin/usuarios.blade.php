...
@section('content')
    <div class="bg-white p-8 rounded-md w-full">
        ...
        <div class="flex justify-between items-center md:ms-10 md:me-10">
                ...
                <div class="flex flex-wrap mt-4">
                    ...
                    <label class="flex items-center ml-4">
                        <select name="biological_gender" class="w-full rounded-md border border-gray-300 py-2 pr-7 focus:outline-none focus:border-indigo-500">">
                            <option value="todos" {{ isset($input['biological_gender']) && $input['biological_gender'] == 'todos' ? 'selected' : '' }}>Todos</option>
                            <option value="masc" {{ isset($input['biological_gender']) && $input['biological_gender'] == 'masc' ? 'selected' : '' }}>Masculino</option>
                            <option value="fem" {{ isset($input['biological_gender']) && $input['biological_gender'] == 'fem' ? 'selected' : '' }}>Femenino</option>
                            <option value="otro" {{ isset($input['biological_gender']) && $input['biological_gender'] == 'otro' ? 'selected' : '' }}>Otro</option>
                        </select>
                    </label>
                    ...
                    <label class="flex items-center ml-4">
                        <select name="role" class="w-full rounded-md border border-gray-300 py-2 pr-7 focus:outline-none focus:border-indigo-500">">
                            ...
                            <option value="todos" {{ isset($input['role']) && $input['role'] == 'todos' ? 'selected' : '' }}>Todos</option>

                            @foreach ($roles as $role)

                                <option value="{{ $role->id }} {{ isset($input['role']) && $input['role'] == $role->id ? 'selected' : '' }}">{{ $role->name }}</option>
            ...
            <form action="{{ route('createUser') }}" class="-mt-16" method="GET">
