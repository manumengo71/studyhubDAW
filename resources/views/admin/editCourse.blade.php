    ...
    <x-slot name="slot">
        <div class="flex items-center justify-center p-12">
            <div class="mx-auto w-full">
                <form action="{{ route('admin.updateCourse', $courseInfo->id) }}" method="POST"
                        ...
                        <div class="md:flex md:space-x-4">
                            <div class="md:w-1/5">
                                ...
                                <select name="language" id="language"
                                    ...
                                    <option value="Alemán" {{ $courseInfo->language == 'Alemán' ? 'selected' : '' }}>
                                        Alemán</option>
                                    <option value="Árabe" {{ $courseInfo->language == 'Árabe' ? 'selected' : '' }}>
                                        Árabe</option>
                                    <option value="Bengalí" {{ $courseInfo->language == 'Bengalí' ? 'selected' : '' }}>
                                        Bengalí</option>
                                    <option value="Chino (Mandarín)"
                                        {{ $courseInfo->language == 'Chino (Mandarín)' ? 'selected' : '' }}>Chino
                                        (Mandarín)</option>
                                    <option value="Coreano" {{ $courseInfo->language == 'Coreano' ? 'selected' : '' }}>
                                        Coreano</option>
                                    <option value="Español" {{ $courseInfo->language == 'Español' ? 'selected' : '' }}>
                                        Español</option>
                                    <option value="Francés" {{ $courseInfo->language == 'Francés' ? 'selected' : '' }}>
                                        Francés</option>
                                    <option value="Hindi" {{ $courseInfo->language == 'Hindi' ? 'selected' : '' }}>
                                        Hindi</option>
                                    <option value="Inglés" {{ $courseInfo->language == 'Inglés' ? 'selected' : '' }}>
                                        Inglés</option>
                                    <option value="Italiano"
                                        {{ $courseInfo->language == 'Italiano' ? 'selected' : '' }}>Italiano</option>
                                    <option value="Japonés" {{ $courseInfo->language == 'Japonés' ? 'selected' : '' }}>
                                        Japonés</option>
                                    <option value="Malayo" {{ $courseInfo->language == 'Malayo' ? 'selected' : '' }}>
                                        Malayo</option>
                                    <option value="Portugués"
                                        {{ $courseInfo->language == 'Portugués' ? 'selected' : '' }}>Portugués</option>
                                    <option value="Ruso" {{ $courseInfo->language == 'Ruso' ? 'selected' : '' }}>Ruso
                                    </option>
                                    <option value="Telugu" {{ $courseInfo->language == 'Telugu' ? 'selected' : '' }}>
                                        Telugu</option>
                                    <option value="Vietnamita"
                                        {{ $courseInfo->language == 'Vietnamita' ? 'selected' : '' }}>Vietnamita
                                    </option>
                                </select>
                                @error('language')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            ...
                            <div class="md:w-1/5">
                                ...
                                <input type="number" name="price" id="price"
                                    value="{{ number_format($courseInfo->price, 2) }}" min="0" max="999"
                                    step="0.01"
                                ...
                                <select name="owner_id" id="owner_id"
                                    ...
                                    @foreach ($users->sortBy('username') as $user)
                                        <option value="{{ $user->id }}"
                                            {{ $courseInfo->owner_id == $user->id ? 'selected' : '' }}>
                                ...
                                <select name="courses_categories_id" id="courses_categories_id"
                                    ...
                                    @foreach ($categories->sortBy('name') as $category)
                                        <option value="{{ $category->id }}"
                                            {{ $courseInfo->courses_categories_id == $category->id ? 'selected' : '' }}>
                            ...
                            <div class="md:w-1/5">
                                ...
                                @endif
                                <input type="hidden" id="imageCourse-remove" name="imageCourse-remove"
                                    value="0">
                            </div>
                ...
                <div class="mb-5">
                    <div class="md:flex md:space-x-4">
                        ...
                        <div x-data="accordion(1)"
                            class="md:w-full mt-4 relative transition-all duration-700 border-2  rounded-xl hover:shadow-2xl">
                                    ...
                                    @foreach ($lessons as $lesson)
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex items-center">
                                                <form action="{{ route('admin.editLesson', $lesson->id) }}"
                                                ...
                                                <form action="{{ route('admin.deleteLesson', $lesson->id) }}"
